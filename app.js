...
const basicFields = Object.fromEntries(
  BASIC_FIELD_IDS.map((id) => [id, document.getElementById(id)]),
);
const advFields = Object.fromEntries(
  ADV_FIELD_IDS.map((id) => [id, document.getElementById(id)]),
);
...
if (ratesEffectiveEl)
  ratesEffectiveEl.textContent = "Rates effective: " + POLICY.effectiveDate;
...
function closeAlertOnOverlay(e) {
...
function isHoliday(ymd) {
  if (
    Object.prototype.hasOwnProperty.call(extraHoursState.customHolidays, ymd)
  ) {
...
// ─── Pay periods ───
function payPeriodStart(year, month) {
  return new Date(year, month, 16);
}
function payPeriodEnd(year, month) {
...
function loadExtraHoursState() {
  ...
  if (typeof data.basicSalary === "number")
    extraHoursState.basicSalary = data.basicSalary;
  if (typeof data.compAllowance === "number")
    extraHoursState.compAllowance = data.compAllowance;
  if (data.customHolidays && typeof data.customHolidays === "object")
    extraHoursState.customHolidays = data.customHolidays;
  if (data.days && typeof data.days === "object") {
    ...
    Object.keys(data.days).forEach(function (ymd) {
      ...
      let shifts = {};
      if (Array.isArray(entry.shifts)) {
        entry.shifts.forEach(function (c) {
          shifts[c] = null;
        });
      } else if (typeof entry.shifts === "object") {
        shifts = entry.shifts;
      ...
      if (Object.keys(shifts).length)
        days[ymd] = { shifts: shifts, autoFilled: entry.autoFilled === true };
    });
    extraHoursState.days = days;
  }
  if (
    data.viewPeriod &&
    typeof data.viewPeriod.year === "number" &&
    typeof data.viewPeriod.month === "number"
  ) {
    extraHoursState.viewPeriod = {
      year: data.viewPeriod.year,
      month: data.viewPeriod.month,
    };
  }
}
...
function saveExtraHoursState() {
  ...
  _ehSaveTimer = setTimeout(function () {
    safeSet(STORAGE_KEY_EXTRA, JSON.stringify(extraHoursState));
  }, 250);
}
...
function saveBasicFields() {
  ...
  _basicSaveTimer = setTimeout(function () {
    ...
    BASIC_FIELD_IDS.forEach(function (id) {
      data[id] = basicFields[id] ? basicFields[id].value : "";
    });
...
function loadBasicFields() {
  ...
  BASIC_FIELD_IDS.forEach(function (id) {
    if (basicFields[id] && typeof data[id] === "string")
      basicFields[id].value = data[id];
  });
  if (
    basicFields["basic-3pm-count"] &&
    !basicFields["basic-3pm-count"].value &&
    "basic-sp1" in data
  )
    basicFields["basic-3pm-count"].value = String(
      Math.floor(Math.max(0, legacySp1)),
    );
  if (
    basicFields["basic-10pm-count"] &&
    !basicFields["basic-10pm-count"].value &&
    "basic-sp2" in data
  )
    basicFields["basic-10pm-count"].value = String(
      Math.floor(Math.max(0, legacySp2)),
    );
  if (
    basicFields["basic-10pm-after-3pm-count"] &&
    !basicFields["basic-10pm-after-3pm-count"].value
  )
    basicFields["basic-10pm-after-3pm-count"].value = String(
      Math.floor(legacy10After3),
    );
}
...
function setMode(m, animate) {
  ...
  if (calendarSection)
    calendarSection.classList.toggle("hidden", m === "BASIC");
  crosscheckEl.classList.remove("visible");
  crosscheckList.innerHTML = "";
  positionSegmentedThumb(
    modeSegmented,
    modeThumb,
    m === "BASIC" ? modeBasicBtn : modeAdvancedBtn,
    animate,
  );
  saveMode();
...
function gatherCalendarAllowanceCounts() {
  ...
  for (let d = new Date(start); d <= end; d = addDays(d, 1)) {
    ...
    if (has3 && has10) {
      ...
      if (d3 === "SHORT" && d10 === "SHORT")
        counts.inferredDoubleDaysShort += 1;
...
function setShiftInputsReadOnly() {
  const all = ADV_FIELD_IDS.map(function (id) {
  ...
  all.forEach(function (el) {
    ...
    el.setAttribute(
      "title",
      "Derived from calendar. Update the calendar to change counts.",
    );
  });
}
...
function computeAllowance(modeArg) {
  ...
  const c = gatherCalendarAllowanceCounts();
  const shortTaxiUnits = Math.max(
    0,
    c.short3 + c.short10 - c.inferredDoubleDaysShort * 2,
  );
  const longTaxiUnits = Math.max(
    0,
    c.long3 + c.long10 - c.inferredDoubleDaysLong * 2,
  );
  ...
  return {
    rSP1: advanced.rSP1,
    rSP2: advanced.rSP2,
    rMeal: advanced.rMeal,
    rTaxi: advanced.rTaxi,
    total: advanced.total,
    distLabel: "Short + Long",
  };
}
...
function highlightIssue(issue) {
  ...
  Array.prototype.forEach.call(
    document.querySelectorAll(".cal-day--warn-pulse"),
    function (el) {
  ...
  const first = document.querySelector(
    '.cal-day[data-ymd="' + issue.dates[0] + '"]',
  );
  if (first)
...
function tweenMoney(el, to) {
  tweenValue(el, to, function (v) {
    return fmt(v);
  });
}
function tweenHours(el, to) {
  tweenValue(el, to, function (v) {
    return fmtHrs(v);
  });
}
...
function renderLive() {
  ...
  if (showNetPayInput && showNetPayInput.checked && liveNetEl) {
    ...
    tweenValue(liveNetEl, netBreakdown.netMonthly, function (v) {
      return "Estimated net pay: " + fmt(v);
    });
  } else if (liveNetEl) {
  ...
  cell.addEventListener("click", function () {
    if (!inPeriod) {
      ...
      if (
        ...
        (target.year === cur.year && target.month > cur.month)
      ) {
...
function openDayModal(ymd) {
  ...
  dayHolidayLbl.textContent = builtIn
    ? "Public holiday (built-in)"
    : "Public holiday";
  syncDayModalToggles();
  ...
  requestAnimationFrame(function () {
    SHIFT_ORDER.forEach(function (code) {
      ...
      const sel =
        wrap.querySelector('.segmented-option[aria-selected="true"]') ||
        wrap.querySelector(".segmented-option");
...
function syncDayModalToggles() {
  ...
  SHIFT_ORDER.forEach(function (code) {
    ...
    if (on && mode === "ADVANCED") {
      ...
      Array.prototype.forEach.call(
        distWrap.querySelectorAll(".segmented-option"),
        function (opt) {
          opt.setAttribute(
            "aria-selected",
            opt.getAttribute("data-dist") === dist ? "true" : "false",
          );
        },
      );
      const seg = distWrap.querySelector(".segmented-control");
      const thumb = distWrap.querySelector(".segmented-thumb");
      const sel = distWrap.querySelector(
...
SHIFT_ORDER.forEach(function (code) {
  dayToggleEls[code].addEventListener("click", function () {
    ...
    if (turnedOn && autoFillRotation) {
      if (hasAutoFilledDays()) {
        showAlert(
          "Autofill already exists on this calendar. Clear autofilled days first before starting a new autofill sequence.",
        );
      } else {
  ...
  const wrap = dayDistEls[code];
  Array.prototype.forEach.call(
    wrap.querySelectorAll(".segmented-option"),
    function (opt) {
...
function submitCombined() {
  ...
  if (mode === "BASIC") {
    ...
    if (!rel.valid) {
      showAlert(
        "Extra 10PM after same-day 3PM cannot exceed either 3PM shifts or 10PM shifts.",
      );
      return;
    }
  }
  ...
  const issues = crossCheck();
  const basePay = {
    basic: extraHoursState.basicSalary || 0,
    comp: extraHoursState.compAllowance || 0,
  };
  lastSnapshot = {
    mode: mode,
    allowance: allowance,
    ...
    extra: extra,
    issues: issues,
    period: extraHoursState.viewPeriod,
    capturedAt: new Date(),
  };
...
function renderResultsPage(snap) {
  resEls.eyebrow.textContent =
    snap.mode === "ADVANCED" ? "Advanced snapshot" : "Basic snapshot";
  ...
  resEls.grand.textContent = fmt(
    round2(snap.allowance.total + baseBasic + baseComp + snap.extra.totalPay),
  );
...
function copyResults() {
  ...
  if (!snap) return;
  const date = snap.capturedAt.toLocaleDateString("en-JM", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  ...
  const grandTotal = round2(
    snap.allowance.total + baseBasic + baseComp + snap.extra.totalPay,
  );
  const lines = [
    "Night Shift — " + date,
...
function exportData() {
  const payload = {
    basic: safeGet(STORAGE_KEY_BASIC),
    adv: safeGet(STORAGE_KEY_ADV),
    mode: safeGet(STORAGE_KEY_MODE),
    extra: safeGet(STORAGE_KEY_EXTRA),
    autofill: safeGet(STORAGE_KEY_AUTOFILL),
...
function importDataFromFile(file) {
  ...
  reader.onload = function () {
    try {
      ...
      if (typeof d.autofill === "string")
        safeSet(STORAGE_KEY_AUTOFILL, d.autofill);
      mode = loadMode();
      loadBasicFields();
      loadAdvFields();
      loadExtraHoursState();
      ...
      basicSalaryInput.value =
        extraHoursState.basicSalary > 0 ? extraHoursState.basicSalary : "";
      compAllowanceInput.value =
        extraHoursState.compAllowance > 0 ? extraHoursState.compAllowance : "";
      setMode(mode, false);
      renderCalendar();
      renderLive();
      showAlert("Import complete.");
    } catch (e) {
      showAlert("Import failed: invalid backup file.");
    }
  };
...
function resetAll() {
  showConfirm(
    "Clear ALL data: counts, base pay, calendar, and holiday overrides?",
    function () {
      BASIC_FIELD_IDS.forEach(function (id) {
        if (basicFields[id]) {
          basicFields[id].value = "";
          basicFields[id].classList.remove("error", "field-input--warn");
        }
      });
      ADV_FIELD_IDS.forEach(function (id) {
        advFields[id].value = "";
        advFields[id].classList.remove("error", "field-input--warn");
      });
      extraHoursState = {
        basicSalary: 0,
        compAllowance: 0,
...
function goBack() {
  showPage(prevPage || pageCalc, "back");
}
function goBackFromInfo() {
  showPage(infoReturnPage || pageCalc, "back");
}
function showPage(el, direction) {
  if (el === pageAbout || el === pageDisclaimer)
    infoReturnPage = getCurrentActivePage();
  ...
  if (!reducedMotion()) {
    if (direction === "back") el.classList.add("page--enter-from-left");
      ...
      el.classList.add("page--enter-from-right");
  }
...
const basicInputs = BASIC_FIELD_IDS.map(function (id) {
  return basicFields[id];
});
const advInputs = ADV_FIELD_IDS.map(function (id) {
  return advFields[id];
});
setShiftInputsReadOnly();
attachInputHandlers(basicInputs.concat(advInputs));
basicInputs.forEach(function (el) {
  el.addEventListener("input", function () {
    saveBasicFields();
    renderLive();
  });
});
advInputs.forEach(function (el) {
  el.addEventListener("input", function () {
    renderLive();
  });
});

modeBasicBtn.addEventListener("click", function () {
  setMode("BASIC", true);
});
modeAdvancedBtn.addEventListener("click", function () {
...
modeBasicBtn.addEventListener("keydown", function (e) {
  onSegmentedNav(e, "BASIC", "ADVANCED", setMode);
});
modeAdvancedBtn.addEventListener("keydown", function (e) {
  onSegmentedNav(e, "BASIC", "ADVANCED", setMode);
});
...
calNextBtn.addEventListener("click", function () {
  ...
  if (
    nextPeriod.year > current.year ||
    (nextPeriod.year === current.year && nextPeriod.month > current.month)
  ) {
    showAlert("You can only view up to the current pay period.");
    return;
  }
...
window.addEventListener("resize", function () {
  requestAnimationFrame(function () {
    positionSegmentedThumb(
      modeSegmented,
      modeThumb,
      mode === "BASIC" ? modeBasicBtn : modeAdvancedBtn,
      false,
    );
  });
});
...
loadExtraHoursState();
loadAutoFillSetting();
if (extraHoursState.basicSalary > 0)
  basicSalaryInput.value = extraHoursState.basicSalary;
if (extraHoursState.compAllowance > 0)
  compAllowanceInput.value = extraHoursState.compAllowance;
mode = loadMode();
setMode(mode, false);
renderCalendar();
syncAllowanceInputsFromCalendar();
