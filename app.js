    // ─── Theme (tri-state: Auto / Light / Dark) ───
    ...
    function applyThemeMode(m) {
      ...
      // Auto: half-moon glyph; Light: sun; Dark: crescent moon
      const icon = m === 'auto' ? '◐' : (dark ? '\uD83C\uDF19' : '\u2600\uFE0F');
      themeBtn.innerHTML = '<span aria-hidden="true">' + icon + '</span>';
      const nextLabel = m === 'auto' ? 'Light' : (m === 'light' ? 'Dark' : 'Auto');
      themeBtn.setAttribute('aria-label', 'Theme: ' + m.charAt(0).toUpperCase() + m.slice(1) + '. Tap to switch to ' + nextLabel + '.');
      themeBtn.classList.toggle('theme-btn--auto', m === 'auto');
      if (themeMetaTag) themeMetaTag.content = dark ? '#171614' : '#01696f';
    }
    ...
    (function initTheme() {
      ...
      if (window.matchMedia) {
        ...
        const onChange = function() { if (themeMode === 'auto') applyThemeMode('auto'); };
